<template>
	<view>
		<!-- 帖子详情页详细信息 -->
		<common-list :item="info" isdetail @follow="follow" @togglesupport="doSupport" @doComment="doComment"
			@doShare="doShare">
			<view>{{info.content}}</view>
			<view>
				<image v-for="(item,index) in images" @click="preview(index)" :src="item.url" class="w-100"
					mode="widthFix"></image>
			</view>
		</common-list>
					...
					for (var i = 0; i < arr.length; i++) {
						...
						if (item.fid == id) {
							...
							temp.push({
								...
								time: $Time.gettime(item.create_time),
								data: item.data,
							});
